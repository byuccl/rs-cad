...
class AutoArtix7SitePackUnitGenerator(val device: Device) : SitePackUnitGenerator() {
	...
	companion object {
		...
		@JvmStatic fun main(args: Array<String>) {
			...
			//val packUnits = AutoArtix7SitePackUnitGenerator(device).buildFromDevice(device, belCosts)
			val packUnits = AutoArtix7SitePackUnitGenerator(device).buildFromDevice(device)
